...
for l = 0:Bvar
    ...
    for i = 1:length(SNR)
        ...
        for k =1:nc
            ...
            D2(:,k) = median((abs(y(:,((k-1)*nb)+1:k*nb))).^2,2); 
...
%% STs
    ...
    pe1 = (muD1+muD0)./(nb*(sD1.^2 - sD0.^2));K = sD0 ./ sD1;
    ...
    TS = (D2 .* repmat(ts1,1,nc)) + repmat(ts2,1,nc) + log(1/beta);
    ...
    PLD6 = sum(double(TS(:,1:round(ph0*nc)) <= 0),2)/(ph0*nc);
    PFD6 = sum(double(TS(:,(round(ph0*nc))+1:end) >= 0),2)/(nc-(round(ph0*nc)));
...
subplot(3,2,6);
plot(SNR,APeso(:,1),'g',SNR,APeso(:,2),'--r',SNR,APeso(:,3),':k','LineWidth',5);
